...
class EmployeeController(BaseController, MongoMixin):
    ...
    @property
    def notification_domain_whitelist(self):
        try:
            return self._config.domains_whitelist(
                whitelist_key='new_employee_email')
        except etcd.EtcdKeyNotFound:
            return []
    ...
    def send_new_employee_email(self, organization_id, organization_name,
        ...
        for domain in self.notification_domain_blacklist:
            if is_match(domain, user_email):
                return
        domain_whitelist = self.notification_domain_whitelist
        if domain_whitelist:
            in_whitelist = any(filter(
                lambda x: is_match(x, user_email), domain_whitelist))
            if not in_whitelist:
                return
