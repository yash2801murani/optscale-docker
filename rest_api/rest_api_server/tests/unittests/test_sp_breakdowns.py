...
class TestSpBreakdownApi(TestApiBase):
    ...
    def test_without_cloud_account_id(self):
        ...
        code, response = self.client.sp_breakdown_get(
            self.org_id, **params)
    ...
    def test_not_found_cloud_acc(self):
        ...
        code, response = self.client.sp_breakdown_get(self.org_id, **filters)
    ...
    def test_invalid_organization(self):
        ...
        for org_id in [organization2['id'], 'impostor']:
            code, response = self.client.sp_breakdown_get(org_id, **filters)
            self.assertEqual(code, 404)
    ...
    def test_date_ranges(self):
        ...
        result = {
            str(self.start_ts): [{
                ...
                'sp_usage_hrs': 0.1,
                'sp_overprovision': 1,  # 2 - 1
                'sp_overprovision_hrs': {
                    't2.large': 2  # 1 / 0.5
                },
                'sp_cost_with_offer': 1,
                'sp_cost_without_offer': 2,
        ...
        result = {
            ...
            str(day1_ts): [{
                ...
                'cloud_account_type': self.cloud_acc1['type'],
                'cloud_account_name': self.cloud_acc1['name'],
                'total_usage_hrs': 0.5,  # 0.2 + 0.3
                'sp_usage_hrs': 0.2,
                'sp_overprovision': 1,  # 4 - 3 + 7 - 7
                'sp_overprovision_hrs': {
                    't2.large': 2  # 1 / 0.5
                },
                'sp_cost_with_offer': 3,
                'sp_cost_without_offer': 4,
                'cost_with_offer': 10,  # 3 + 7
                'cost_without_offer': 12,  # 4 + 8
            }],
            str(day2_ts): [{
                'cloud_account_id': self.cloud_acc1['id'],
                'cloud_account_type': self.cloud_acc1['type'],
    ...
    def test_date_expected(self):
        ...
        empty_stats['sp_overprovision_hrs'] = {'t2.large': 0}
        result = {
            str(self.start_ts): [
                {
                    'cloud_account_id': self.cloud_acc1['id'],
                    ...
                    'sp_cost_with_offer': 22,
                    'sp_cost_without_offer': 22,
                    'cost_with_offer': 22,
                    'cost_without_offer': 22,
                }
            ],
            str(day1_ts): [empty_stats],
            str(day2_ts): [empty_stats],
        }
        self.assertDictEqual(response['breakdown'], result)

    def test_post(self):
        code, response = self.client.post(
            self.client.sp_breakdown_url(self.org_id), body={})
    ...
    def test_delete(self):
        code, response = self.client.delete(
            self.client.sp_breakdown_url(self.org_id))
        self.assertEqual(code, 405)
