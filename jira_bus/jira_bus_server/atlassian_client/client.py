...
from jira_bus.jira_bus_server.atlassian_client.decorators import _wrap_request_errors
from jira_bus.jira_bus_server.atlassian_client.exceptions import (
...
class _AtlassianClientRequestBuilder:
    ...
    def __getattr__(self, attr):
        if attr in self.HTTP_METHODS:
            return lambda r_data=None, r_json=None, **kwargs: self._client.request(
                method=attr,
                url=self._url,
                params=kwargs,
                data=r_data,
