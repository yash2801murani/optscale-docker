...
class TestEmailApi(TestHeraldBase):
    ...
    def test_validation_email_payload(self):
        ...
        code, response = self.client_v2.email_send(email=invalid_email, subject="subj")
        self.assertEqual(400, code)
        self.assertEqual("invalid email", response["error"]["reason"])
        code, response = self.client_v2.email_send(email=valid_email, subject="subj")
        self.assertEqual(400, code)
        self.assertEqual("invalid email", response["error"]["reason"])
        code, response = self.client_v2.email_send(email=[valid_email], subject="subj", template_type=None)
        self.assertEqual(400, code)
        self.assertEqual("template_type is not provided", response["error"]["reason"])
        code, response = self.client_v2.email_send(
            email=[valid_email], subject="subj", template_type="not_existing_type"
        )
        self.assertEqual(400, code)
        self.assertEqual("invalid template_type", response["error"]["reason"])
        invalid_template_params = "template_params"
        code, response = self.client_v2.email_send(
            email=[valid_email], subject="subj", template_params=invalid_template_params
        )
        self.assertEqual(400, code)
    ...
    def test_email_template_params(self):
        ...
        subject = "subj"
        code, response = self.client_v2.email_send(
            email=[valid_email], subject=subject, template_type=template_type, template_params=None
        )
        self.assertEqual(201, code)
        result = {
            "email": [valid_email],
        ...
        code, response = self.client_v2.email_send(
            email=[valid_email], subject=subject, template_type=template_type, template_params={}
        )
        self.assertEqual(201, code)
        result = {
            "email": [valid_email],
