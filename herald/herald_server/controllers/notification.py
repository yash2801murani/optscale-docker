...
from herald.herald_server.models.models import Field, FilterCriteria, Notification, Reaction
...
class NotificationController(BaseController):
    ...
    def _validate_reactions(reactions):
        if reactions is not None and (
            not isinstance(reactions, list) or any(filter(lambda x: not isinstance(x, dict), reactions))
        ):
    ...
    def _parse_filter(self, filter_str):
        ...
        for criteria_str in filter_str.split(" "):
            try:
                ...
                raise WrongArgumentsException(Err.G0013, [criteria_str])
    ...
    def create(self, **kwargs):
        ...
        for reaction in reactions:
            notification.reactions.append(ReactionController(self.session, self._config).create(**reaction))
    ...
    def edit(self, item_id, **kwargs):
        ...
        if reactions is not None:
            notification.reactions = [
                ReactionController(self.session, self._config).create(**reaction) for reaction in reactions
            ]
    ...
    def _event_user_reactions(self, event):
        ...
        for field in filtering_fields:
            ...
            event_based_filters.append(
                and_(FilterCriteria.field_id == field.id, FilterCriteria.value == event_field_value)
            )

        query = (
            self.session.query(Notification.user_id, Reaction)
            .filter(
                Notification.deleted.is_(False),
            )
            .join(Notification.criterias)
            .filter(or_(*event_based_filters))
            .join(Notification.reactions)
        )
    ...
    def _filter_user_access(self, event, user_reactions):
        ...
        return [reaction for user_id, reaction in user_reactions if user_id in authorized_user_ids]
