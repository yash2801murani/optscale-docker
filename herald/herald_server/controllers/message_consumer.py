...
class Consumer:
    ...
    def connect(self):
        ...
        self._connection = TornadoConnection(
            self.connection_parameters, self.on_connection_open, self.on_connection_error
        )
    ...
    def on_connection_closed(self, connection, reply_code):
        self._channel = None
        LOG.warning("Connection closed, reopening in 5 seconds: (%s)", reply_code)
        self._connection.ioloop.add_timeout(5, self.connect)
    ...
    def setup_dlx_exchange(self):
        ...
        self._channel.exchange_declare(self.dlx_exchange, callback=self.on_dlx_exchange_declareok)
    ...
    def setup_queue(self):
        ...
        self._channel.queue_declare(self.queue, callback=self.on_queue_declareok, durable=True)

    def on_queue_declareok(self, unused_frame):
        LOG.info("Queue declared successfully")
        self._channel.queue_bind(self.queue, exchange=self.dlx_exchange, callback=self.on_queue_bind_ok)
    ...
    def _publish_message(self, message, queue):
        ...
        self._channel.basic_publish("", queue, json.dumps(message), properties=pika.BasicProperties(delivery_mode=2))
